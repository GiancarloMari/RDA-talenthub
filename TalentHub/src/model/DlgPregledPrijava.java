...
public class DlgPregledPrijava extends JDialog {
    ...
    public DlgPregledPrijava(User poslodavac) {
        ...
        JButton btnOdbaci = new JButton("Odbaci prijavu");
        btnOdbaci.setBackground(new Color(220, 57, 57));
        btnOdbaci.setForeground(Color.WHITE);
        btnOdbaci.setFont(new Font("Segoe UI", Font.BOLD, 14));
        btnOdbaci.setFocusPainted(false);
        btnOdbaci.setPreferredSize(new Dimension(200, 35));
        btnOdbaci.addActionListener(e -> obrisiPrijavu());

        JPanel bottomPanel = new JPanel(new FlowLayout(FlowLayout.CENTER, 10, 10));
        bottomPanel.setBackground(mainPanel.getBackground());
        bottomPanel.add(btnZaposli);
        bottomPanel.add(btnOdbaci);
    ...
    private void obrisiPrijavu() {
        int red = tblPrijave.getSelectedRow();
        if (red == -1) {
            JOptionPane.showMessageDialog(this, "Odaberi prijavu za brisanje.");
            return;
        }
        ...
        try (Connection conn = DB.connect()) {
            PreparedStatement stmt = conn.prepareStatement(
                "DELETE FROM prijava WHERE id_projekt = ? AND id_freelancer = ?");
            stmt.setInt(1, projekt.getId());
            stmt.setInt(2, idFreelancer);
            stmt.executeUpdate();

            JOptionPane.showMessageDialog(this, "Prijava je obrisana.");
            ucitajPrijave(); // Osvježi tablicu
        } catch (Exception e) {
            e.printStackTrace();
            JOptionPane.showMessageDialog(this, "Greška pri brisanju prijave.");
        }
    }
}
